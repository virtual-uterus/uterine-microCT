...
MaskPath = '../RawImages/BinaryForeground/';
Njm = 756; Nim = 1196; Nkm = 500;
...
dXs = double(fread(fid,prod(N),'double'));
dYs = double(fread(fid,prod(N),'double'));
dZs = double(fread(fid,prod(N),'double'));
h2Xs = double(fread(fid,prod(N),'double'));
hXYs = double(fread(fid,prod(N),'double'));
hXZs = double(fread(fid,prod(N),'double'));
h2Ys = double(fread(fid,prod(N),'double'));
hYZs = double(fread(fid,prod(N),'double'));
h2Zs = double(fread(fid,prod(N),'double'));
fclose(fid);
...
for k=kstart:kend,
  fnamein = sprintf('%sh03_%03d.png',MaskPath,k);
  M = (imread(fnamein)==1);
